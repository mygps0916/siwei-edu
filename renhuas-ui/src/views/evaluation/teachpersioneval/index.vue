<template>
  <div class="app-container">
    <el-form :model="queryParams" ref="queryForm" :inline="true" v-show="showSearch" label-width="68px">
      <el-form-item label="老师个人报告名称" prop="evalPersionName">
        <el-input
          v-model="queryParams.evalPersionName"
          placeholder="请输入老师个人报告名称"
          clearable
          size="small"
    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['evaluation:teachpersioneval:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['evaluation:teachpersioneval:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['evaluation:teachpersioneval:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['evaluation:teachpersioneval:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="teachpersionevalList" @selection-change="handleSelectionChange">
      <el-table-column type="selection" width="55" align="center" />
      <el-table-column label="序号" align="center" prop="evalId" />
      <el-table-column label="老师个人报告名称" align="center" prop="evalPersionName" />
    ...
    <el-dialog :title="title" :visible.sync="open" width="500px" append-to-body>
      <el-form ref="form" :model="form" :rules="rules" label-width="80px">
        <el-form-item label="老师个人报告名称" prop="evalPersionName">
          <el-input v-model="form.evalPersionName" placeholder="请输入老师个人报告名称" />
        </el-form-item>
        <el-form-item label="学业阶段" prop="academic">
...
<script>
import { listTeachpersioneval, getTeachpersioneval, delTeachpersioneval, addTeachpersioneval, updateTeachpersioneval, exportTeachpersioneval } from "@/api/evaluation/teachpersioneval";

export default {
  name: "Teachpersioneval",
  data() {
    return {
      // 遮罩层
      loading: true,
      ...
      rules: {
        evalPersionName: [
          { required: true, message: "老师个人报告名称不能为空", trigger: "blur" }
        ],
        academic: [
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listTeachpersioneval(this.queryParams).then(response => {
        this.teachpersionevalList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getTeachpersioneval(evalId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.evalId != null) {
            updateTeachpersioneval(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addTeachpersioneval(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除老师个人测评编号为"' + evalIds + '"的数据项？').then(function() {
        return delTeachpersioneval(evalIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有老师个人测评数据项？').then(() => {
        this.exportLoading = true;
        return exportTeachpersioneval(queryParams);
      }).then(response => {
