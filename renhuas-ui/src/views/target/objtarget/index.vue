    ...
    <el-row :gutter="10" class="mb8">
      <el-col :span="1.5">
        <el-button
          ...
          size="mini"
          @click="handleAdd"
          v-hasPermi="['target:objtarget:add']"
        >新增</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="single"
          @click="handleUpdate"
          v-hasPermi="['target:objtarget:edit']"
        >修改</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          :disabled="multiple"
          @click="handleDelete"
          v-hasPermi="['target:objtarget:remove']"
        >删除</el-button>
      </el-col>
      <el-col :span="1.5">
        <el-button
          ...
          @click="handleExport"
          v-hasPermi="['target:objtarget:export']"
        >导出</el-button>
      </el-col>
      <right-toolbar :showSearch.sync="showSearch" @queryTable="getList"></right-toolbar>
    </el-row>

    <el-table v-loading="loading" :data="objtargetList" @selection-change="handleSelectionChange">
...
<script>
import { listObjtarget, getObjtarget, delObjtarget, addObjtarget, updateObjtarget, exportObjtarget } from "@/api/target/objtarget";
...
export default {
  ...
  methods: {
    ...
    getList() {
      this.loading = true;
      listObjtarget(this.queryParams).then(response => {
        this.objtargetList = response.rows;
    ...
    handleUpdate(row) {
      ...
      getObjtarget(tarId).then(response => {
      ...
      this.$refs["form"].validate(valid => {
        if (valid) {
          if (this.form.tarId != null) {
            updateObjtarget(this.form).then(response => {
              this.$modal.msgSuccess("修改成功");
              this.open = false;
              this.getList();
            });
          } else {
            addObjtarget(this.form).then(response => {
    ...
    handleDelete(row) {
      ...
      this.$modal.confirm('是否确认删除客观指标编号为"' + tarIds + '"的数据项？').then(function() {
        return delObjtarget(tarIds);
      }).then(() => {
        this.getList();
    ...
    handleExport() {
      ...
      this.$modal.confirm('是否确认导出所有客观指标数据项？').then(() => {
        this.exportLoading = true;
        return exportObjtarget(queryParams);
      }).then(response => {
